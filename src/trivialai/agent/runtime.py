...
def _parse_decision(content: Any) -> Dict[str, Any]:
    """
    Parse one agent-loop decision.

    Parsing is deliberately tolerant of common model JSON formatting through
    util.loadch(), while the decision envelope itself remains strict.
    """
    ...
    if decision_type == "tool-call":
        ...
        # Runtime metadata such as tool_call_id/step/attempt is generated by
        # the loop itself. Models sometimes echo those fields from execution
        # history; normalize them away instead of rejecting an otherwise valid
        # semantic decision.
        return {
            "type": "tool-call",
            "tool": parsed["tool"],
