...
class MemoryFile:
    """
    A file or directory that the agent may read, and optionally write.
    """
...
class MemoryURL:
    """
    A URL the agent may consult (fetching is up to host tools / caller).
    """
...
class MemoryText:
    """
    A raw text snippet the agent may treat as reference material.
    """
...
MemoryItem = Union[MemoryFile, MemoryURL, MemoryText]


@dataclass
class AgentScope:
    """
    Encodes where the agent is allowed to write.
    - root: base working directory
    - scratch_dir: always writable
    - allowed_writes: extra directories/files we explicitly grant write access to
    """

    root: Path
    scratch_dir: Path
    allowed_writes: List[Path] = field(default_factory=list)

    def can_write(self, target: Path) -> bool:
        target = target.resolve()
        # scratchpad is always ok
        if target == self.scratch_dir or self.scratch_dir in target.parents:
            return True
        # explicitly allowed paths (directories or files)
        for p in self.allowed_writes:
            p = p.resolve()
...
class AgentStep:
    ...
    kind: "do" | "loop" (for now; can be extended)
    description: user-level instruction or pointer to a prompt file
    stop_for_confirm: future hook, e.g., for confirmation steps
    """

    kind: str
    description: str
    stop_for_confirm: bool = False
...
class Agent:
    ...
    def __init__(
        ...
        *,
        ...
        self.scope = AgentScope(root=root_path, scratch_dir=scratch)
        self.memory_items: List[MemoryItem] = []
        self.tools: Tools = Tools()
        self.steps: List[AgentStep] = []
        self._next_step_index: int = 0  # index into self.steps

        # Event history (used when late-building context)
        self.history_events: List[Dict[str, Any]] = []

        # NDJSON log of all runs
